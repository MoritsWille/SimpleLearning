...
namespace SimpleLearning
{
    ...
    class Program
    {
        ...
        sstart:
            ...
            string CurrentRead = Console.ReadLine();
            if (CurrentRead == "RUN")
            {
            start:
                for (int t = 1; t < 5; t++)
        ...
        else if(CurrentRead == "RANDOMIZE")
            {
                for (int e = 0; e < 9; e++)
                {
                    xDoc.SelectSingleNode("weights/w" + e.ToString()).InnerText = Convert.ToString((random.Next(0, 200) - 100) * 0.01);
                }
                xDoc.Save(Environment.CurrentDirectory + "/XORsettings.xml");
                goto sstart;
            }
